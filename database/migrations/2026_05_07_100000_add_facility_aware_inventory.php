...
return new class extends Migration {
    public function up(): void
    {
        ...
        if (! Schema::hasTable('warehouse_facility_schemes')) {
        ...
        if (! Schema::hasTable('customer_facility_schemes')) {
        ...
        $defaultSchemes = [
            ['code' => 'REGULAR', 'name' => 'Regular', 'description' => 'Default unrestricted facility.', 'is_restricted' => false, 'requires_tracking' => true, 'requires_reporting' => false],
            ['code' => 'BPJS', 'name' => 'BPJS Program', 'description' => 'BPJS restricted stock.', 'is_restricted' => true, 'requires_tracking' => true, 'requires_reporting' => true],
            ['code' => 'KEK_VAT_EXEMPT', 'name' => 'KEK VAT Exempt', 'description' => 'KEK bebas PPN.', 'is_restricted' => true, 'requires_tracking' => true, 'requires_reporting' => true],
            ['code' => 'HIBAH', 'name' => 'Hibah', 'description' => 'Grant stock.', 'is_restricted' => true, 'requires_tracking' => true, 'requires_reporting' => true],
            ['code' => 'CONSIGNMENT', 'name' => 'Consignment', 'description' => 'Consignment stock.', 'is_restricted' => true, 'requires_tracking' => true, 'requires_reporting' => false],
        ];

        foreach ($defaultSchemes as $scheme) {
            DB::table('facility_schemes')->updateOrInsert(
                ['code' => $scheme['code']],
                array_merge($scheme, ['updated_at' => now(), 'created_at' => now()])
            );
        }

        $regularId = DB::table('facility_schemes')->where('code', 'REGULAR')->value('id');
        foreach ($tables as $tableName) {
            if (Schema::hasTable($tableName) && Schema::hasColumn($tableName, 'facility_scheme_id') && $regularId) {
