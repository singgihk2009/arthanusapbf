...
useEffect(()=>{let c=false; setLoading(true); fetch(route(`apps.procurement.vendors.${currentTab}`, vendor.id)).then(r=>r.json()).then(d=>!c&&setData(d)).finally(()=>!c&&setLoading(false)); return ()=>c=true;},[vendor.id,currentTab]);
const TabComp=useMemo(()=>components[currentTab]||OverviewTab,[currentTab]);
return <div className='bg-white border rounded-lg'>
<div className='flex flex-wrap gap-2 border-b p-3'>{tabs.map(([key,label])=><button key={key} onClick={()=>router.get(route('apps.procurement.vendors.show',vendor.id),{tab:key},{preserveState:true,preserveScroll:true})} className={`px-3 py-1 rounded ${currentTab===key?'bg-indigo-600 text-white':'bg-gray-100'}`}>{label}</button>)}</div>
