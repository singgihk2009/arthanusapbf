      ...
      {showForm && (
        <form onSubmit={submit} className='border rounded p-3 bg-gray-50 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm mb-4'>
          ...
          <div className='md:col-span-2 flex items-center gap-2'>
            ...
            <button type='button' className='px-3 py-1 rounded border text-sm' onClick={resetForm}>
              Cancel
            </button>
          </div>
        </form>
      ...
      <div className='overflow-auto mt-2'>
        <table className='min-w-full text-sm border'>
          <thead className='bg-gray-50'>
            <tr>
              {['Full Name', 'Position', 'Email', 'Mobile/Phone', 'Status', 'Action'].map((h) => (
          ...
          <tbody>
            {contacts.length === 0 && (
              <tr>
                <td colSpan={6} className='p-3 text-gray-500'>No contacts found.</td>
              </tr>
            )}
            {contacts.map((pc) => (
              <tr key={pc.id} className='border-b'>
                <td className='p-2'>{pc.contact?.full_name || '-'}</td>
                <td className='p-2'>{pc.contact?.position_title || '-'}</td>
                <td className='p-2'>{pc.contact?.email || '-'}</td>
                <td className='p-2'>{pc.contact?.mobile || pc.contact?.phone || '-'}</td>
                <td className='p-2'>{pc.status}</td>
                <td className='p-2 space-x-2'>
                  <button type='button' onClick={() => startEdit(pc)} className='text-xs text-indigo-600'>Edit</button>
                  <button
                    type='button'
                    onClick={() => router.delete(`/apps/procurement/vendors/${vendor.id}/party-contacts/${pc.id}`, {
                      preserveScroll: true,
                      onSuccess: () => {
                        onRefresh?.();
