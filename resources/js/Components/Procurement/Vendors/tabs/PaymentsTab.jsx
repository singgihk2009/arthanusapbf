  ...
  return <div className='space-y-4'>
    ...
    <div className='overflow-auto border rounded'>
      <table className='min-w-full text-sm'>
        ...
        <tbody>{payments.map(p => <tr key={p.id} className='border-t'><td className='px-3 py-2'><Link className='text-indigo-600' href={`/apps/procurement/vendors/${vendor.id}/payments/${p.id}`}>{p.payment_no}</Link></td><td className='px-3 py-2'>{formatDate(p.payment_date)}</td><td className='px-3 py-2'>{p.status}</td><td className='px-3 py-2 text-right'>{formatCurrency(p.total_invoice_amount)}</td><td className='px-3 py-2 text-right'>{formatCurrency(p.total_wht_amount)}</td><td className='px-3 py-2 text-right'>{formatCurrency(p.stamp_duty_amount)}</td><td className='px-3 py-2 text-right'>{formatCurrency(p.freight_amount)}</td><td className='px-3 py-2 text-right'>{formatCurrency(p.bank_charge_amount)}</td><td className='px-3 py-2 text-right'>{formatCurrency(p.net_vendor_payment_amount)}</td><td className='px-3 py-2 text-right'>{formatCurrency(p.total_cash_out_amount)}</td><td className='px-3 py-2 whitespace-nowrap space-x-1 text-center'><Link href={`/apps/procurement/vendors/${vendor.id}/payments/${p.id}`} className='px-2 py-1 bg-gray-100 rounded'>View</Link><Link href={`/apps/procurement/vendors/${vendor.id}/payments/${p.id}/edit`} className={`px-2 py-1 bg-gray-100 rounded ${String(p.status).toUpperCase() !== 'DRAFT' ? 'pointer-events-none opacity-50' : ''}`}>Edit</Link><button type='button' onClick={() => approvePayment(p.id)} disabled={String(p.status).toUpperCase() !== 'SUBMITTED'} className='px-2 py-1 bg-gray-100 rounded disabled:opacity-50'>Approve</button><button type='button' onClick={() => deletePayment(p.id)} disabled={String(p.status).toUpperCase() !== 'DRAFT'} className='px-2 py-1 bg-red-50 text-red-700 rounded disabled:opacity-50'>Delete</button></td></tr>)}</tbody>
      </table>
    </div>
  </div>;
}
