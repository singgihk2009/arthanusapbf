...
export default function DocumentsTab({ vendor }) {
  ...
  const submitUpload = (documentTypeId) => {
    ...
    router.post(route('apps.procurement.vendors.documents.upload', vendor.id), payload, {
      forceFormData: true,
      preserveScroll: true,
      onSuccess: () => {
        fileInput.value = '';
  ...
  const verifyDocument = (documentId, verificationStatus) => {
    router.post(
      route('apps.procurement.vendors.documents.verify', [vendor.id, documentId]),
      ...
      return <tr key={r.id}>
        ...
        <td className='border px-2'>
          <div className='space-y-1'>
            ...
            <input ref={(el) => { fileInputs.current[r.document_type_id] = el; }} type='file' accept='.pdf,.jpg,.jpeg,.png' className='w-full' />
          </div>
        </td>
        <td className='border px-2'>
          <div className='flex flex-wrap gap-1'>
            <button type='button' onClick={() => submitUpload(r.document_type_id)} className='rounded border border-blue-300 px-2 py-1 text-xs text-blue-700'>{d ? 'Replace' : 'Upload'}</button>
