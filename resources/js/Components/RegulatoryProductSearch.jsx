...
export default function RegulatoryProductSearch({ selectedProduct, onSelect, onClear }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-gray-200 p-3 dark:border-gray-800">
        ...
        {results.length > 0 && <div className="max-h-64 overflow-auto rounded border border-gray-100 dark:border-gray-700"><table className="min-w-full text-xs"><thead><tr><th>Source</th><th>Kode BPOM/Source Code</th><th>NIE</th><th>Nama Produk</th><th>Produsen</th><th>Sediaan</th><th>Kekuatan</th><th>Komoditi</th></tr></thead><tbody>{results.map((r)=><tr key={r.id} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" onClick={()=>onSelect(r)}><td>{r.source_name}</td><td>{r.source_code ?? '-'}</td><td>{r.nie}</td><td>{r.product_name_source}</td><td>{r.industry_name}</td><td>{r.dosage_form}</td><td>{r.strength}</td><td>{r.commodity_type}</td></tr>)}</tbody></table></div>}
        {selectedProduct && <div className="rounded border border-emerald-200 bg-emerald-50 p-3 text-sm dark:border-emerald-900 dark:bg-emerald-900/20"><div className="font-medium">Selected Regulatory Product</div><div>{selectedProduct.source_name} - {selectedProduct.source_code ?? selectedProduct.nie}</div><div>NIE: {selectedProduct.nie ?? '-'}</div><div>{selectedProduct.product_name_source}</div><button type="button" className="mt-2 text-xs text-rose-600" onClick={onClear}>Clear Regulatory Reference</button></div>}
    </div>);
}
