...
export default function Index({ batches, purposes }) {
    ...
    const handleSubmit = (event) => {
        ...
        const submitRoute = repairBatch
            ? route('apps.setup.manual-sales-integration.imports.retry', repairBatch.id)
            : route('apps.setup.manual-sales-integration.imports.store');
    ...
    const deleteBatch = (batch) => {
        ...
        window.axios.post(route('apps.setup.manual-sales-integration.imports.discard', batch.id))
    ...
    const commitBatch = (batchId) => {
        if (!window.confirm('Commit batch ini? Proses akan membuat Sales Order, Fulfillment/Shipment, Customer Invoice, Collection, dan allocation AR tanpa jurnal akunting.')) return;
        setCommittingId(batchId);
        window.axios.post(route('apps.setup.manual-sales-integration.imports.commit', batchId))
            .then(() => window.location.reload())
    ...
    return (
        <>
            <Head title="Manual Sales Integration" />
            <div className="space-y-5 p-6">
                ...
                <div ref={uploadSectionRef} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-900 dark:bg-gray-950">
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                        ...
                        <a href={route('apps.setup.manual-sales-integration.template.excel')} className="rounded-lg border border-indigo-500 px-4 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/30">
                                        ...
                                        <tr key={batch.id} className="border-t border-gray-100 dark:border-gray-900">
                                            ...
                                            <td className="px-3 py-2">
                                                <div className="flex flex-wrap justify-center gap-2">
                                                    <button type="button" onClick={() => window.open(route('apps.setup.manual-sales-integration.imports.show', batch.id), '_blank')} className="rounded border border-gray-300 px-2 py-1 text-xs">JSON Detail</button>
