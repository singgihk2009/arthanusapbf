...
export default function Edit() {
    const { barcode, items } = usePage().props;
    const { data, setData, post, errors } = useForm({ item_id: barcode.item_id, barcode: barcode.barcode, note: barcode.note ?? '', _method: 'PUT' });

    const submit = (e) => {
        e.preventDefault();
        post(route('apps.master-data.barcodes.update', barcode.id));
    };

    return (
        <>
            <Head title="Ubah Barcode" />
            <Card title="Ubah Barcode" icon={<IconFileBarcode size={20} strokeWidth={1.5} />} form={submit} footer={<Button type="submit" label="Simpan" icon={<IconPencilPlus size={20} strokeWidth={1.5} />} variant="gray" />}>
