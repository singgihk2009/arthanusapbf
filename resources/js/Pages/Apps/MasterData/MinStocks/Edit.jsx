...
export default function Edit() {
    const { minStock, items, warehouses } = usePage().props;
    const { data, setData, post, errors } = useForm({ warehouse_id: minStock.warehouse_id, item_id: minStock.item_id, min_stock_base: minStock.min_stock_base, _method: 'PUT' });

    const submit = (e) => {
        e.preventDefault();
        post(route('apps.master-data.min-stocks.update', minStock.id));
    };

    return (
        <>
            <Head title="Ubah Min Stock" />
            <Card title="Ubah Min Stock" icon={<IconListCheck size={20} strokeWidth={1.5} />} form={submit} footer={<Button type="submit" label="Simpan" icon={<IconPencilPlus size={20} strokeWidth={1.5} />} variant="gray" />}>
