...
export default function Create() {
    ...
    const { data, setData, post, errors } = useForm({ name: '', parent_id: '' });

    const submit = (e) => {
        e.preventDefault();
        post(route('apps.master-data.categories.store'));
    };

    return (
        <>
            <Head title="Tambah Category" />
            <Card title="Tambah Category" icon={<IconCategory size={20} strokeWidth={1.5} />} form={submit} footer={<Button type="submit" label="Simpan" icon={<IconPencilPlus size={20} strokeWidth={1.5} />} variant="gray" />}>
