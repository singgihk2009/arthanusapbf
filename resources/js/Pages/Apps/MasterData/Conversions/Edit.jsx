...
export default function Edit() {
    const { conversion, items, uoms } = usePage().props;
    const { data, setData, post, errors } = useForm({ item_id: conversion.item_id, from_uom_id: conversion.from_uom_id, to_uom_id: conversion.to_uom_id, factor: conversion.factor, _method: 'PUT' });

    const submit = (e) => {
        e.preventDefault();
        post(route('apps.master-data.conversions.update', conversion.id));
    };

    return (
        <>
            <Head title="Ubah Konversi UOM" />
            <Card title="Ubah Konversi UOM" icon={<IconExchange size={20} strokeWidth={1.5} />} form={submit} footer={<Button type="submit" label="Simpan" icon={<IconPencilPlus size={20} strokeWidth={1.5} />} variant="gray" />}>
