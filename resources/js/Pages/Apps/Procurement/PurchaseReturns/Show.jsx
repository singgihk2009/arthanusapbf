    ...
    <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-900 dark:bg-gray-950">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">{purchaseReturn.return_date} | {purchaseReturn.vendor_label} | {purchaseReturn.reason_category}</p>
          <p className="text-sm text-gray-600">GR: {purchaseReturn.receiving_number} | Warehouse: {purchaseReturn.warehouse_label}</p>
        </div>
      ...
      {purchaseReturn.deduction && <div className="rounded border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">
        Deduction: <b>{purchaseReturn.deduction.deduction_no}</b> | Amount {money(purchaseReturn.deduction.amount)} | Applied {money(purchaseReturn.deduction.applied_amount)} | Remaining Credit {money(purchaseReturn.deduction.remaining_amount)} | Invoice {purchaseReturn.deduction.invoice_number || '-'}
      </div>}
      ...
      <div className="overflow-x-auto rounded border border-gray-200 dark:border-gray-800">
        <table className="min-w-full text-sm"><thead className="bg-gray-50"><tr>{['Item','Batch','Expired','Qty','Unit Cost','Amount','Reason','Notes'].map((h) => <th key={h} className="px-3 py-2 text-left">{h}</th>)}</tr></thead><tbody>
          {purchaseReturn.lines.map((line) => <tr key={line.id} className="border-t"><td className="px-3 py-2">{line.item_name}</td><td className="px-3 py-2">{line.batch_number || '-'}</td><td className="px-3 py-2">{line.expired_date || '-'}</td><td className="px-3 py-2 text-right">{money(line.qty_returned)}</td><td className="px-3 py-2 text-right">{money(line.unit_cost)}</td><td className="px-3 py-2 text-right">{money(line.line_amount)}</td><td className="px-3 py-2">{line.reason}</td><td className="px-3 py-2">{line.condition_notes || '-'}</td></tr>)}
