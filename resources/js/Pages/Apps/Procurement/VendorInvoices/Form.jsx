...
export default function Page({ vendor, receivingLines, internalInvoiceNoPreview }) {
  ...
  const toggle = (line, checked) => {
    if (checked) setData('lines',[...data.lines,{receipt_line_id:line.receipt_line_id,qty_invoiced:line.qty_available_to_invoice,unit_price:line.unit_price}]);
    else setData('lines',data.lines.filter(x=>x.receipt_line_id!==line.receipt_line_id));
  };
  ...
  return <AppLayout><div className='p-6 space-y-6'>
    ...
    <div className='bg-white p-4 border rounded overflow-auto'>
      <table className='min-w-full text-sm'><thead><tr><th></th><th>Receiving</th><th>PO</th><th>Item</th><th>Qty Rec</th><th>Qty Inv</th><th>Qty Avail</th><th>Qty To Invoice</th><th>Unit Price</th><th>Total</th></tr></thead><tbody>{receivingLines.map(l=>{const s=data.lines.find(x=>x.receipt_line_id===l.receipt_line_id); return <tr key={l.receipt_line_id} className='border-t'><td><input type='checkbox' checked={!!s} onChange={e=>toggle(l,e.target.checked)}/></td><td>{l.receiving_no}</td><td>{l.po_no}</td><td>{l.item_name}</td><td>{l.qty_received}</td><td>{l.qty_already_invoiced}</td><td>{l.qty_available_to_invoice}</td><td><input disabled={!s} type='number' value={s?.qty_invoiced??''} onChange={e=>setData('lines',data.lines.map(x=>x.receipt_line_id===l.receipt_line_id?{...x,qty_invoiced:e.target.value}:x))} className='border p-1 w-24'/></td><td><input disabled={!s} type='number' value={s?.unit_price??''} onChange={e=>setData('lines',data.lines.map(x=>x.receipt_line_id===l.receipt_line_id?{...x,unit_price:e.target.value}:x))} className='border p-1 w-28'/></td><td>{(((+s?.qty_invoiced||0)*(+s?.unit_price||0))).toFixed(2)}</td></tr>})}</tbody></table>
    </div>
