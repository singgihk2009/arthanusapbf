    ...
    return <>
        ...
        <Card title={isEdit ? 'Edit Purchase Order' : 'Create Purchase Order'} form={submit} footer={<div className='flex items-center gap-2'><Button type='submit' label='Save Draft' disabled={processing} variant='gray' /><button type='button' onClick={handleBack} className='rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50'>Back</button>{isDirty && <span className='text-xs text-amber-600'>Data belum disimpan.</span>}</div>}>
            ...
            <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                ...
                <div className='flex flex-col gap-2'><label className='text-sm text-gray-600'>Header Facility</label><select value={data.facility_scheme_id} onChange={(e) => setData('facility_scheme_id', e.target.value)} className='w-full rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700'><option value=''>-</option>{facilitySchemes.map((f)=><option key={f.id} value={f.id}>{f.code} - {f.name}</option>)}</select></div><div className='flex flex-col gap-2'><label className='text-sm text-gray-600'>No Referensi Facility</label><input value={data.facility_reference_no} onChange={(e) => setData('facility_reference_no', e.target.value)} placeholder={(facilityMap[String(data.facility_scheme_id)]?.code === 'KEK_VAT_EXEMPT') ? 'No Referensi PPKEK' : 'No Referensi'} className='w-full rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300' /></div><div className='flex flex-col gap-2'><label className='text-sm text-gray-600'>Tanggal Facility</label><input type='date' value={data.facility_reference_date} onChange={(e) => setData('facility_reference_date', e.target.value)} className='w-full rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300' /></div><div className='flex flex-col gap-2'><label className='text-sm text-gray-600'>Notes</label><textarea value={data.notes} onChange={(e) => setData('notes', e.target.value)} className='w-full rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300' /></div>
            </div>
