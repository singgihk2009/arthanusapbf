...
export default function Form() {
  ...
  const { data, setData, post, errors } = useForm({
    ...
    address: vendor?.address ?? '', province: vendor?.province ?? '',
  ...
  return <><Head title='Master Vendor'/><Card title='Master Vendor' form={submit} footer={<Button type='submit' label='Simpan' variant='gray'/>}><div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
    ...
    <Input label='Type Vendor' value={data.vendor_type} onChange={e=>setData('vendor_type', e.target.value)} errors={errors.vendor_type}/><Input label='Provinsi' value={data.province} onChange={e=>setData('province', e.target.value)} errors={errors.province}/>
