...
const emptyLine = { item_id: '', batch_id: '', qty_requested: '', uom_id: '' };
...
export default function Edit() {
    ...
    const updateLine = (index, field, value) => setForm((prev) => {
        ...
        const updatedLine = { ...updatedLines[index], [field]: value };
    ...
    const submit = async (event) => {
        ...
        try {
            await window.axios.put(route('apps.transfer.warehouse.update', entry.id), form);
            setMessage({ type: 'success', text: 'Transfer antar gudang berhasil diperbarui.' });
        } catch (error) {
            if (error.response?.status === 422) {
                setErrors(error.response.data.errors ?? {});
                setMessage({ type: 'error', text: 'Validasi gagal. Cek field yang ditandai.' });
            } else {
                setMessage({ type: 'error', text: 'Gagal memperbarui transfer antar gudang.' });
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
        <>
            <Head title="Edit Transfer Antar Gudang" />

            <div className="space-y-4">
                <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-900 dark:bg-gray-950">
                    <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Edit Transfer Antar Gudang</h2>
                    <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">Ubah dokumen transfer sebelum diposting.</p>

                    <form onSubmit={submit} className="space-y-4">
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
                            <div><label className="mb-1 block text-sm text-gray-700 dark:text-gray-300">Gudang Asal</label><select value={form.from_warehouse_id} onChange={(e) => updateHeader('from_warehouse_id', e.target.value)} className={inputClassName}><option value="">Pilih Gudang Asal</option>{warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.code} - {warehouse.name}</option>)}</select>{errors.from_warehouse_id && <p className="mt-1 text-xs text-red-500">{errors.from_warehouse_id[0]}</p>}</div>
                            <div><label className="mb-1 block text-sm text-gray-700 dark:text-gray-300">Gudang Tujuan</label><select value={form.to_warehouse_id} onChange={(e) => updateHeader('to_warehouse_id', e.target.value)} className={inputClassName}><option value="">Pilih Gudang Tujuan</option>{warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.code} - {warehouse.name}</option>)}</select>{errors.to_warehouse_id && <p className="mt-1 text-xs text-red-500">{errors.to_warehouse_id[0]}</p>}</div>
                        ...
                        <div className="overflow-x-auto rounded border border-gray-200 dark:border-gray-800">
                            <table className="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
                                <thead className="bg-gray-50 dark:bg-gray-900"><tr><th className="px-2 py-2 text-left">Item</th><th className="px-2 py-2 text-left">Metode Valuasi</th><th className="px-2 py-2 text-left">Qty Transfer</th><th className="px-2 py-2 text-left">UOM</th><th className="px-2 py-2 text-left">Aksi</th></tr></thead>
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                    {form.lines.map((line, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="p-2"><select value={line.batch_id} onChange={(e) => updateLine(index, 'batch_id', e.target.value)} className={`w-52 ${lineInputClassName}`} disabled={!line.item_id}><option value="">Tanpa batch (AVG)</option>{batches.filter((batch) => String(batch.item_id) === String(line.item_id)).map((batch) => <option key={batch.id} value={batch.id}>{batch.batch_no}{batch.expired_date ? ` (EXP ${batch.expired_date})` : ''}</option>)}</select>{errors[`lines.${index}.batch_id`] && <p className="mt-1 text-xs text-red-500">{errors[`lines.${index}.batch_id`][0]}</p>}</td>
                                            <td className="p-2"><input type="number" min="0" step="0.000001" value={line.qty_requested} onChange={(e) => updateLine(index, 'qty_requested', e.target.value)} className={`w-28 ${lineInputClassName}`} />{errors[`lines.${index}.qty_requested`] && <p className="mt-1 text-xs text-red-500">{errors[`lines.${index}.qty_requested`][0]}</p>}</td>
                        ...
                        <button type="submit" disabled={loading} className="rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50 dark:bg-gray-100 dark:text-gray-900">{loading ? 'Menyimpan...' : 'Update Transfer'}</button>
                    </form>
                </div>
