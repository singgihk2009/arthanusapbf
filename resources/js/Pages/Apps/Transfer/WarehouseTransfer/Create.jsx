...
export default function Create() {
    ...
    const submit = async (event) => {
        ...
        try {
            await window.axios.post(route('apps.transfer.warehouse.store'), form);
            setMessage({ type: 'success', text: 'Transfer antar gudang berhasil disimpan.' });
            setForm((prev) => ({
                ...prev,
                from_warehouse_id: '',
                to_warehouse_id: '',
            ...
            if (error.response?.status === 422) {
                ...
                setMessage({ type: 'error', text: 'Gagal menyimpan transfer antar gudang.' });
            }
        } finally {
            setLoading(false);
        }
        ...
        <>
            ...
            <div className="space-y-4">
                <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-900 dark:bg-gray-950">
                    <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Transfer Antar Gudang</h2>
                    <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">Pindahkan stok barang dari gudang asal ke gudang tujuan.</p>
                    ...
                    <form onSubmit={submit} className="space-y-4">
                        ...
                        <button type="submit" disabled={loading} className="rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50 dark:bg-gray-100 dark:text-gray-900">{loading ? 'Menyimpan...' : 'Simpan Transfer'}</button>
                    </form>
                </div>
