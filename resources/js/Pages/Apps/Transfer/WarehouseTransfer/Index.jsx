...
export default function Index() {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('Yakin hapus transfer ini?')) {
            return;
        }

        setProcessingId(id);
        try {
            await window.axios.delete(route('apps.transfer.warehouse.destroy', id));
    ...
    const handlePost = async (id) => {
        if (!window.confirm('Posting transfer ini? Stok akan dipindahkan antar gudang.')) {
            return;
        }

        setProcessingId(id);
        try {
            await window.axios.post(route('apps.inventory.posting.transfer', id));
            window.location.reload();
        } finally {
            setProcessingId(null);
        }
    ...
    const handleUnpost = async (id) => {
        ...
        setProcessingId(id);
        try {
            await window.axios.post(route('apps.inventory.unposting.transfer', id));
            window.location.reload();
        } finally {
            setProcessingId(null);
        }
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-900 dark:bg-gray-950">
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                        <div>
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">List dokumen pemindahan stok antar gudang.</p>
                        </div>
                        <Link href={route('apps.transfer.warehouse.create')} className="rounded-lg bg-gray-900 px-3 py-2 text-sm font-medium text-white dark:bg-gray-100 dark:text-gray-900">Add Transfer</Link>
                    </div>
                    ...
                    <div className="overflow-x-auto rounded border border-gray-200 dark:border-gray-800">
                        <table className="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {entries.data.length === 0 && <tr><td colSpan={7} className="px-3 py-4 text-center text-gray-500">Belum ada data transfer antar gudang.</td></tr>}
                                {entries.data.map((entry, idx) => (
                                    <tr key={entry.id} className="text-gray-800 dark:text-gray-100">
                                        ...
                                        <td className="px-3 py-2">{entry.from_warehouse_label}</td>
                                        <td className="px-3 py-2">{entry.to_warehouse_label}</td>
                                        <td className="px-3 py-2"><span className="rounded border border-gray-300 px-2 py-1 text-xs">{entry.status}</span></td>
                                        <td className="px-3 py-2">
                                            <div className="flex justify-center gap-2">
                                                <Link href={route('apps.transfer.warehouse.edit', entry.id)} className="rounded border border-gray-300 px-2 py-1 text-xs">Edit</Link>
                                                {entry.status !== 'RECEIVED' && <button type="button" onClick={() => handlePost(entry.id)} disabled={processingId === entry.id} className="rounded border border-blue-300 px-2 py-1 text-xs text-blue-700 disabled:opacity-50">Post</button>}
                                                {entry.status === 'RECEIVED' && <button type="button" onClick={() => handleUnpost(entry.id)} disabled={processingId === entry.id} className="rounded border border-amber-300 px-2 py-1 text-xs text-amber-700 disabled:opacity-50">Unpost</button>}
