...
export default function Page({ customer, salesOrder, warehouses = [], priceList }) {
  ...
  const patchLine=(i,patch)=>{setData('lines',(prev)=>{const ls=[...prev];ls[i]={...ls[i],...patch};const l=ls[i];const gross=+l.qty_sold*(+l.unit_price);const disc=gross*(+l.discount_percent)/100;const tax=(gross-disc)*(+l.tax_percent)/100;ls[i].discount_amount=disc;ls[i].tax_amount=tax;ls[i].line_total=gross-disc+tax;return ls;});};
