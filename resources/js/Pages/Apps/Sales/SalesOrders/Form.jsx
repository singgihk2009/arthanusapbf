...
export default function Page({ customer, salesOrder, warehouses = [], priceList, priceListSource }) {
  ...
  const searchItems=async(i,text)=>{patchLine(i,{search_text:text,suggestions:[]}); if(text.length<3) return; const r=await axios.get(route('apps.items.search'),{params:{q:text,warehouse_id:data.warehouse_id}}); patchLine(i,{suggestions:r.data||[]});};
