...
const makeLine = () => ({ item_id:'', item_label:'', search_text:'', batch_id:'', uom_id:'', uom_name:'', available_stock:null, cogs:0, qty_sold:1, unit_price:0, discount_percent:0, tax_percent:11, discount_amount:0, tax_amount:0, line_total:0, notes:'', price_list_id:null, price_list_line_id:null, batch_options:[], suggestions:[], is_searching:false, searched:false });
...
export default function Page({ customer, salesOrder, warehouses = [], priceList, priceListSource }) {
  ...
  const searchItems=async(i,text)=>{
    patchLine(i,{search_text:text,suggestions:[],searched:false,is_searching:text.length>=3});
    if(text.length<3) return;
    const r=await axios.get(route('apps.items.search'),{params:{q:text,warehouse_id:data.warehouse_id}});
    patchLine(i,{search_text:text,suggestions:r.data||[],is_searching:false,searched:true});
  };
