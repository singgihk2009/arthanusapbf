...
export default function Page({ customer, summary, salesOrders = [], documentTypes = [] }) {
  ...
  const approvableSalesOrderIds = useMemo(
    () => salesOrders.filter((salesOrder) => String(salesOrder.status || '').toLowerCase() === 'submitted').map((salesOrder) => salesOrder.id),
    [salesOrders],
  );
