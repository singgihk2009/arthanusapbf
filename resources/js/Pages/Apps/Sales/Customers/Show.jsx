                          ...
                          {customerInvoices.map((invoice) => {
                            ...
                            return (
                              <tr key={invoice.id} className={flowInvoiceIds.includes(Number(invoice.id)) ? 'bg-indigo-50/40' : ''}>
                                ...
                                <td className='px-3 py-2 text-center'><div className='flex justify-center gap-1'><Link href={route('apps.customer-invoices.show', invoice.id)} className='rounded border border-slate-300 px-2 py-1 text-xs'>View</Link>{statusKey(invoice.status) === 'draft' && <Link href={route('apps.customer-invoices.edit', invoice.id)} className='rounded border border-indigo-300 px-2 py-1 text-xs text-indigo-700'>Edit</Link>}</div></td>
                              </tr>
                            );
                          })}
                        </tbody>
