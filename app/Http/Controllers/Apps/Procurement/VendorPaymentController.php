...
class VendorPaymentController extends Controller
{
    ...
    public function create(Vendor $vendor){ return Inertia::render('Apps/Procurement/VendorPayments/Form',['vendor'=>$vendor,'outstandingInvoices'=>$this->outstandingInvoices($vendor)]); }
    ...
    public function edit(Vendor $vendor, VendorPayment $payment){ abort_unless($payment->can_edit, 422, 'Only draft can be edited'); return Inertia::render('Apps/Procurement/VendorPayments/Form',['vendor'=>$vendor,'payment'=>$payment->load('lines'),'outstandingInvoices'=>$this->outstandingInvoices($vendor)]); }
    public function store(StoreVendorPaymentRequest $request, Vendor $vendor){ $payment = DB::transaction(fn()=> $this->upsertPayment(new VendorPayment(),$vendor,$request->validated())); return to_route('apps.procurement.vendor-payments.show',$payment)->with('success','Payment draft saved.'); }
    public function update(UpdateVendorPaymentRequest $request, Vendor $vendor, VendorPayment $payment){ abort_unless($payment->can_edit, 422); DB::transaction(fn()=> $this->upsertPayment($payment,$vendor,$request->validated())); return back()->with('success','Payment updated.'); }
