...
class VendorController extends Controller
{
    ...
    public function store(StoreVendorRequest $request){ DB::transaction(function () use ($request) { $vendor = Vendor::query()->create(array_merge($request->validated(), ['created_by' => auth()->id(), 'updated_by' => auth()->id()])); $this->upsertContacts($vendor, $request->validated()); }); return redirect('/apps/procurement/vendors'); }
    ...
    public function update(UpdateVendorRequest $request, Vendor $vendor){ DB::transaction(function () use ($request, $vendor) { $vendor->update(array_merge($request->validated(), ['updated_by' => auth()->id()])); $this->upsertContacts($vendor, $request->validated()); }); return redirect('/apps/procurement/vendors'); }
