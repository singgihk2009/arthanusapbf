...
public function show(Sale $salesOrder,Request $r){abort_unless($r->user()?->can('sales-order.view'),403);$salesOrder->load(['customer.priceList','warehouse','priceList','lines.item.baseUom','lines.batch','lines.uom','lines.facilityScheme','shipments']);$salesOrder->lines->transform(function($line) use ($salesOrder){$available=$this->availabilityService->getAvailableStock((int)$line->item_id,$salesOrder->warehouse_id);$line->available_stock=$available;$line->stock_status=$this->availabilityService->stockStatus($available,(float)$line->qty_sold);return $line;});$relatedDispatches=DB::table('internal_usages')->where(function($q) use ($salesOrder){$q->where(function($sq) use ($salesOrder){$sq->where('source_type','sales_order')->where('source_id',$salesOrder->id);})->orWhere('sale_id',$salesOrder->id);})->orderByDesc('id')->get(['id','number','document_date','status','posted_at','sales_order_synced_at','source_type','sale_id']);return Inertia::render('Apps/Sales/SalesOrders/Show',['salesOrder'=>$salesOrder,'relatedDispatches'=>$relatedDispatches]);}
