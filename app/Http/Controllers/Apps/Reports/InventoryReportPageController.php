    ...
    public function exportStockBalanceExcel(Request $request): BinaryFileResponse
    {
        ...
        if ($isStockPosition) {
            ...
            $xlsxRows = [[
                ...
                $isUsage ? 'Nama Pengirim Barang' : 'Referensi',
                $isUsage ? 'Referensi' : 'Nama Pengirim Barang',
                'Kode Barang',
                'Kategory Barang',
                'Nama Barang',
                'Satuan',
        ...
        foreach ($rows as $row) {
            if ($isStockPosition) {
                ...
                $line = [
                    ...
                    $row->trx_datetime,
                    $isUsage ? $row->vendor_name : $row->po_reference,
                    $isUsage ? $row->po_reference : $row->vendor_name,
                    $row->sku,
                    $row->category_name,
                    $row->item_name,
                    $row->uom_name,
    ...
    private function itemUsageQuery(array $filters)
    {
        ...
        return DB::table('stock_ledgers')
            ...
            ->select([
                ...
                DB::raw("COALESCE(internal_usages.number, '-') as gr_number"),
                ...
                DB::raw("COALESCE(source_purchase_orders.po_number, '-') as po_reference"),
                ...
                DB::raw("COALESCE(internal_usages.outbound_number, '-') as facility_reference_no"),
